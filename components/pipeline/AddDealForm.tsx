...
export function AddDealForm({ funds, members, defaultOwner, defaultFundId }: { funds: { id: string; name: string }[]; members: string[]; defaultOwner: string; defaultFundId: string }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/20 flex items-start justify-center pt-20" onClick={() => setOpen(false)}>
      <form
        ref={formRef}
        className="card p-4 w-[560px] space-y-3"
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2"><label className="lbl">Name *</label><input name="name" className="input" autoFocus required placeholder="Company / opportunity" /></div>
          ...
          {stage === "Passed" && <div className="col-span-2"><label className="lbl">Pass reason *</label><input name="passReason" className="input" required /></div>}
        </div>
        {more ? (
          <div className="grid grid-cols-2 gap-3 pt-2 border-t border-line-2">
            ...
            <div className="col-span-2"><label className="lbl">Fit notes</label><textarea name="fitNotes" className="textarea" /></div>
          </div>
        ) : (
